...
export default function PatientLogin() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check role to ensure it is a PATIENT (or ADMIN)
      ...
      if (profile.role === 'DOCTOR') {
        // Sign out doctor from patient login
        await supabase.auth.signOut();
        setError('This portal is for patients. Doctors must log in through the Doctor Portal.');
        setLoading(false);
        return;
      }
      ...
      <div className="max-w-md w-full space-y-8 bg-white dark:bg-gray-900 p-8 sm:p-10 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-800 transition duration-300">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
            Sign in to manage appointments & talk to AI
          </p>
        </div>
        ...
        <div className="text-center pt-2">
          <p className="text-sm text-gray-600 dark:text-gray-400">
            ...
            <Link href="/register" className="font-semibold text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 transition">
              Create Patient Account
            </Link>
          </p>
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-850">
            <Link href="/doctor/login" className="text-xs font-medium text-gray-450 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-400">
              Are you a doctor? Go to Doctor Portal
            </Link>
          </div>
        </div>
